...
logo = f"""{y}
      ...
      !KKKKKKKKKKKKKp-               -qKKKKKKKKKKKKK!#! /usr/bin/env python3
# Author by @riz4d
...
logo = f"""{y}
      ...
      <KKKKKKKKkr,                       ,rSKKKKKKKK<
       -"v]qj;-                             -;jq]v"-
                       {b}[ I.G.T.P {b}]
           {d}  Info Gathering Toolkit For Pentesting{w}
             {y}Author  : {r}@riz4d{w}
             {y}Queries : {r}https://telegram.me/rizad_x96"""
...
class Facebook():
    ...
    def facedumper(self):
        ...
        if "mbasic_logout_button" in str(req):
            if "What do you think now" in str(req):
                ...
                try:
                    ...
                    x = parser(requests.get(mbasic.format("/riz4d"),cookies=cookies).content,"html.parser").find("a",string="Ikuti")["href"]
    ...
    if len(arg) == 1: menu()
        ...
        if arg[1] in ("update"):
            if which("termux-setup-storage"): path = "$PREFIX/bin/sigit"
                ...
                if os.path.isdir("/usr/local/bin/"): path = "/usr/local/bin/igtpkit"
                else: path = "/usr/bin/igtpkit"
            os.system(f"wget https://raw.githubusercontent.com/riz4d/IGTP/main/igtpkit.py -O {path} && chmod +x {path}")
